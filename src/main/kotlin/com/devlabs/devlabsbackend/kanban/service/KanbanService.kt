...
class KanbanService(
    ...
    fun getOrCreateBoardForProject(projectId: UUID): KanbanBoardResponse {
        ...
        // Initialize project team to avoid LazyInitializationException
        ...
        board.columns.forEach { column ->
            ...
            column.tasks.forEach { task ->
                task.createdBy.name // Initialize user
                task.assignedTo?.name // Initialize assigned user if exists
            }
        }
        
        return board.toBoardResponse()
    }    fun createTask(request: CreateTaskRequest, userId: UUID): KanbanTaskResponse {
    ...
    }    fun updateTask(taskId: UUID, request: UpdateTaskRequest, userId: UUID): KanbanTaskResponse {
        ...
        val savedTask = kanbanTaskRepository.save(task)
        return savedTask.toTaskResponse()
    }    fun moveTask(taskId: UUID, request: MoveTaskRequest, userId: UUID): KanbanTaskResponse {
        val task = kanbanTaskRepository.findById(taskId).orElseThrow {
            NotFoundException("Task with id $taskId not found")
        }
