...
interface TeamRepository : JpaRepository<Team, UUID> {    @Query("SELECT DISTINCT t FROM Team t LEFT JOIN FETCH t.members WHERE t IN (SELECT t2 FROM Team t2 JOIN t2.members m WHERE m = :member)")
    ...
    fun findByMember(@Param("member") member: User, pageable: Pageable): Page<Team>    @Query("SELECT DISTINCT t FROM Team t LEFT JOIN FETCH t.members WHERE LOWER(t.name) LIKE LOWER(CONCAT('%', :name, '%'))")
    ...
    fun findByNameContainingIgnoreCase(@Param("name") name: String, pageable: Pageable): Page<Team>    @Query("SELECT DISTINCT t FROM Team t LEFT JOIN FETCH t.members")
    ...
    @Query("SELECT DISTINCT t FROM Team t LEFT JOIN FETCH t.members LEFT JOIN FETCH t.projects",
           countQuery = "SELECT COUNT(t) FROM Team t")
    fun findAllWithMembersAndProjects(pageable: Pageable): Page<Team>    @Query("SELECT DISTINCT t FROM Team t LEFT JOIN FETCH t.members LEFT JOIN FETCH t.projects WHERE t IN (SELECT t2 FROM Team t2 JOIN t2.members m WHERE m = :member)")
